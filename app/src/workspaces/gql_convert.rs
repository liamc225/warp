...
use warp_graphql::workspace::{
    ...
    TelemetryEnablementSetting as GqlTelemetryEnablementSetting,
...
pub(super) fn organization_telemetry_policy(
    enforced_setting: Option<GqlTelemetryEnablementSetting>,
    force_enabled: bool,
    ...
    match enforced_setting {
        Some(GqlTelemetryEnablementSetting::Enable) => {
            OrganizationTelemetryPolicy::Enforced(TelemetryEnablementSetting::Enabled)
        }
        Some(GqlTelemetryEnablementSetting::Disable) => {
            OrganizationTelemetryPolicy::Enforced(TelemetryEnablementSetting::Disabled)
        }
        Some(GqlTelemetryEnablementSetting::Other(value)) => {
            report_error!(
                anyhow!(
                    "Invalid TelemetryEnablementSetting '{value}'. Make sure to update client GraphQL types!"
                ),
                warp_core::errors::ReportErrorLogMode::OncePerRun
            );
            OrganizationTelemetryPolicy::Unknown
        }
        None if force_enabled => {
            OrganizationTelemetryPolicy::Enforced(TelemetryEnablementSetting::Enabled)
        }
        None => OrganizationTelemetryPolicy::Unmanaged,
    }
}
...
impl From<GqlWorkspaceSettings> for WorkspaceSettings {
    fn from(gql_workspace_settings: GqlWorkspaceSettings) -> WorkspaceSettings {
        let telemetry_policy = organization_telemetry_policy(
            gql_workspace_settings.telemetry_settings.enforced_setting,
